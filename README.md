...
> The migration step needs Postgres reachable on `localhost:5477`. Start it once with `docker compose up -d` if it's not already running.
>
> `bash scripts/dev.sh` does all of this in one shot: brings Postgres up, installs deps, and runs migrations + seeders.
...
To stop Postgres: `Ctrl+C` in its terminal, or `docker compose down` from anywhere. Data persists in a named volume; use `docker compose down -v` to wipe it.
