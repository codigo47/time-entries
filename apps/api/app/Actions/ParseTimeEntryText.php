...
PROMPT],
    ...
    private function resolveNamesToIds(array $row): array
    {
        ...
        return [
            ...
            'company_name' => $company instanceof Company ? $company->name : (($row['company_name'] ?? '') ?: null),
            'employee_id' => $employee?->id,
            'employee_name' => $employee instanceof Employee ? $employee->name : (($row['employee_name'] ?? '') ?: null),
            'project_id' => $project?->id,
            'project_name' => $project instanceof Project ? $project->name : (($row['project_name'] ?? '') ?: null),
            'task_id' => $task?->id,
            'task_name' => $task instanceof Task ? $task->name : (($row['task_name'] ?? '') ?: null),
            'date' => $row['date'] ?? null,
